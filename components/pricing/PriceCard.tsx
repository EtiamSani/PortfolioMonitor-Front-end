...
const PriceCard = ({ title, description, price, items, temporality, itemsNotInTheOffer, cardBorderClassName, buttonTexts, classNameForPrice }:any) => {
    return (
        ...
        <Card className="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white rounded-lg border ">
            ...
            <div className="flex justify-center items-baseline my-8">
                <span className={classNameForPrice}>{price}</span> 
            ...
            <ul role="list" className="mb-8 space-y-4 text-left">
                ...
                {itemsNotInTheOffer && itemsNotInTheOffer.map((itemNotInTheOffer:any, index:any) => (
                    <li key={index} className="flex items-center space-x-3">
                        ...
                        <span className='text-gray-400'>{itemNotInTheOffer}</span>
                    </li>
                ))}
            </ul>
